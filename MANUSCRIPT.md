...
> Draft v0.1 — venue-neutral. Target framing: computational / biological psychiatry
> (e.g. *Molecular Psychiatry*, *Translational Psychiatry*, *Biological Psychiatry: CNNI*,
> *JAMA Psychiatry*). Tables/figures point to reproducible artifacts in `results/` and
> `reports/`. All numbers below are generated by the scripts named in each section.
...
these disorders share genetic risk, neurobiology, symptom expression and treatment
...
two influential research programmes: the dimensional reframing of psychopathology (e.g.
RDoC; the *p*-factor and hierarchical-taxonomy work) and data-driven *biotyping*, which
seeks discrete patient subgroups from clinical and biological data.
...
diagnosis once these are examined.
...
We analysed 9,013 patients enrolled at FACE expert-centre sites: BP 6,252, SZ 2,209 and
DR 552 (Table 1). Patients carried one of seven enrolled DSM subtypes — bipolar I, II and
...
parallel analysis over-extracted (K≈14, reflecting many tiny construct-specific factors),
so the number of dimensions was fixed by **split-half reproducibility**: we refit the model
on random halves and retained the most granular *K* whose dimensions reproduced across
halves (minimum Tucker congruence). Reproducibility was high only at low, even *K* and
...
**Site robustness** was assessed by ComBat harmonization (neuroHarmonize) of the domain
scores across 20 sites with ≥10 patients, re-deriving the dimensions and re-running the
head-to-head (`scripts/robustness_site.py`). The head-to-head was **replicated** using V2
as the follow-up.
...
kind of data and in favour of a dimensional description, echoing RDoC and hierarchical-
taxonomy frameworks but reached here bottom-up from a formal structure test rather than
assumed.
...
**Table 1. Cohort composition.** [V0 n=9,013] BP 6,252 / SZ 2,209 / DR 552; seven enrolled
DSM subtypes (BP-I/II/NOS; schizophrenia/schizoaffective/schizophreniform; major depressive
disorder). Follow-up V1–V4 with attrition (DR excluded at V3 for sparsity). *(Expand with
age, sex, site counts from `results/qa_missingness.csv` and the loader.)*
...
- **Figure 1. Trans-diagnostic structure is dimensional.** Eigenvalue spectrum (no gap),
  monotone gap-vs-null, HDBSCAN↔cohort ARI 0.70, DSM-subtype mood↔psychosis ordering.
  Source: `reports/structure_test.html`.
- **Figure 2. The six-dimension loading structure** (domain × dimension heatmap). Source:
  `reports/dimensional_final.html`.
- **Figure 3. Head-to-head outcome prediction** (DSM vs dimensions vs combined). Source:
  `reports/phase5_V1.html`, `reports/phase5_V2.html`.
- **Figure 4. Trait–state gradient** (per-dimension test–retest V1–V4). Source:
  `reports/longitudinal_axes.html`.
- **Figure 5. Cognitive *g* + processing-speed structure and symptom-dimension
  correlations** (BP/SZ). Source: `reports/cognition_bpsz.html`.
