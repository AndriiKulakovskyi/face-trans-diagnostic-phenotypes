...
the marginalized model (Section~\ref{sec:estimation}), the map is fit \emph{once} and then every patient is
...
\begin{caveatbox}[Uncertainty must propagate to M2]
The scores are a distribution, not a point. A stratification (M2) that hard-assigns patients to clusters on the
...
Two follow-ons remain (M2-prep): full-$N$ projection of the non-Gaussian block (a logistic/count conditional,
