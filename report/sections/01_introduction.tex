...
Section~\ref{sec:results} reports the as-built empirical map (seven dimensions) and the candidate adjudication.
Section~\ref{sec:validation} presents the estimator/prior-robustness confirmation and the cross-cohort
measurement-invariance analysis. Section~\ref{sec:engineering} documents the software: repository structure,
the config-first design, the test suite, and the two engineering bugs found and fixed \emph{with measurement}.
Section~\ref{sec:roadmap} sketches the later milestones into which this document will grow, and
Section~\ref{sec:limitations} states the limitations and open problems. Appendices collect notation, the prior
atlas, the acceptance-gate definitions, the full factor-correlation matrix, and a glossary of instruments.
