...
\begin{enumerate}\setlength{\itemsep}{1pt}
  \item[\textbf{M1}] \textbf{Transdiagnostic dimensions} --- the measurement map on the baseline visit. \emph{This report (\S\ref{sec:results}).}
  \item[\textbf{M2}] \textbf{Validated strata} --- decision regions on the M1 coordinates. \emph{This report (\S\ref{sec:strata}).}
...
This document is the methods-of-record and the as-built record for \textbf{Milestones 1 and 2}, written at a
level of detail beyond a journal article so that it can both seed the eventual manuscript and serve as the
project's living technical reference. Section~\ref{sec:data} describes the data foundation and the
...
map to per-patient coordinates with reliability flags. Section~\ref{sec:strata} presents \textbf{Milestone~2}
--- whether those coordinates harbour discrete biotypes (they do not: a continuum) and the eight extreme
phenotypes, severity spine, and DSM-5 head-to-head that describe it. Section~\ref{sec:engineering} documents
the software: repository structure, the config-first design, the test suite, and the two engineering bugs found
and fixed \emph{with measurement}. Section~\ref{sec:roadmap} sketches the later milestones into which this
document will grow, and Section~\ref{sec:limitations} states the limitations and open problems. Appendices collect notation,
