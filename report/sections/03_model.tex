...
The structure is a \textbf{bifactor / ESEM} (exploratory structural-equation model). \emph{Bifactor} means
every indicator may load on the general factor $G$ \emph{and} on its home specific factor; \emph{ESEM} means
an indicator is additionally \emph{allowed} a small cross-loading onto other factors, with those
cross-loadings shrunk toward zero by the soft priors --- unlike a classical confirmatory factor analysis,
which forbids cross-loadings outright and so forces every instrument into exactly one box. The image to hold
is a hub-and-spoke (Figure~\ref{fig:bifactor}): $G$ is a hub wired (faintly) to every instrument, each
specific factor a spoke wired (strongly) to its own pool; a depression/anxiety ``window'' is an instrument
wired only to the hub.
...
(Chapter~\ref{sec:results} shows it refined the magnitude without changing the conclusion).
\end{definitionbox}
...
onto a shared continuous scale. Forcing a yes/no or a count variable onto a continuous ruler manufactures
covariance that is an artefact of the rescaling and biases the very dimensions we are trying to estimate ---
so each item keeps its natural scale, and the observation likelihood is what carries the variable type,
\begin{equation}
...
marginalized analytically (Chapter~\ref{sec:estimation}); the non-Gaussian block carries explicit latents.}
...
measurement map is the \emph{prior $\to$ posterior} comparison (Chapter~\ref{sec:results}): this theory map placed beside the fitted
