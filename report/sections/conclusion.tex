...
does not see. It \textbf{organizes}: on those coordinates patients form not discrete biotypes but a
graded \emph{continuum}, eight soft archetypes that cut across the DSM-5 subtypes (adjusted Rand
$\approx 0$) while describing the data far better than diagnosis does. It \textbf{persists}: scored
forward onto follow-up without re-estimation, the measurement is invariant and the geometry replays ---
durable biology, moving symptoms. It \textbf{predicts}: a baseline coordinate forecasts two-year
functioning incrementally beyond diagnosis, severity, and the present outcome. And it does \textbf{not},
on the evidence available, \textbf{prescribe}: with treatment data recovered from the per-cohort
...
rather than treatment-as-usual; and any clinical claim would require external
