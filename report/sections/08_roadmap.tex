...
This document is a living artifact. \textbf{Milestones 1, 2, and 3 are complete} (pending PI sign-off) --- the
measurement map, its stratification, and their temporal coherence --- and are the foundation the remaining
milestones build on. Each is a separate milestone with its own acceptance gate, and none begins before the
previous is locked. \textbf{M4 is next.}
...
decomposition and an independent geometric test that agree. The operative read-out for M4: \emph{stratify on
the durable biology (cognition, metabolic, inflammatory); monitor the moving symptoms (severity, suicidality,
sleep)}. Persistence is not prediction --- that is M4.

\paragraph{M4 --- Prognosis.} \planned\ \textbf{The immediate next step.} Does a patient's position on the map carry incremental predictive value
...
\subsection{Milestones 1--3 are complete --- and the small follow-ons they leave}
...
follow-up --- invariant measurement and a replaying spine-versus-corner geometry (Section~\ref{sec:temporal}). What remains is a short, principled list of
...
\textbf{PI sign-off on the adjudication, the atlas, and the strata/temporal findings locks
M1\,$+$\,M2\,$+$\,M3; then M4.}
