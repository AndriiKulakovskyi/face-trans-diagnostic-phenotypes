...
On the harmonized FACE V0 baseline ($N=9{,}013$), the hybrid bifactor/ESEM yields a \textbf{certified
nine-dimension transdiagnostic map}: a general factor $G$ (functional burden) plus eight specific axes ---
...
entanglement} of each domain (Section~\ref{sub:bioperpG} refines biology's value under the correlated-$G$
identification).}
\label{tab:map}
...
\rowcolor{facebg} suicidality & ISF binary ideation/attempt $+$ count & 2.7--3.4 (logit) & 0.3--0.6\\
mania & YMRS 0.57, Altman 0.76 (BP) & 0.49--0.76 & 0.15\\
\rowcolor{facebg} substance & alcohol/cannabis lifetime SUD, nicotine & 0.38--0.83 (logit) & 0.13\\
\bottomrule
\end{tabular}
\end{table}

\subsection{The empirical atlas, beside the theory}
The scientific deliverable of M1 is the \emph{prior $\to$ posterior} comparison
...
inflammatory ($0.07$) burden are nearly independent of $G$, far below cognition ($0.39$) and sleep ($0.44$).
Mania and substance are likewise low ($|\lambda_G|$ $0.15$ and $0.13$).}
\label{fig:biologyg}
\end{figure}

\begin{keyresult}[Biology rides on axes severity does not see]
Freeing $G$ to correlate with the specifics, it correlates $+0.06$ with inflammatory and $+0.14$ with
metabolic, versus $+0.39$ with cognition and $+0.44$ with sleep. So metabolic and inflammatory load carry
$\approx 92$--$98\%$ of their variance \emph{independent} of how impaired a patient is overall. Two patients
who look equally ill on a clinician's global impression can differ sharply in biological load --- precisely the
orthogonal signal a biology-aware stratification (M2) can exploit. \emph{(The clean continuous-backbone
estimate, metabolic$\sim$$G$ $0.12$--$0.14$, supersedes an earlier provisional mixed-fit read of $0.28$; both
...
\certified\ The dimensions are \emph{weakly} correlated --- mean $|\Phi_{\text{off-diagonal}}|\approx 0.10$
...
\textbf{confirmed both as real, low-$G$ axes} and the joint nine-dimension model was re-certified rather than
...
indicators ($+0.38$ to $+0.83$ on the logit scale) under their proper Bernoulli/negative-binomial likelihoods,
...
\certified\ That non-Gaussian psychopathology composes with the continuous biology in one correlated-factor
...
\midrule
\rowcolor{facebg} isf01--05 (ideation) & $+2.7$ to $+3.4$ & $+0.33$ to $+0.56$\\
...
Suicidality & confirmed (mixed) & binary ISF $+2.7$ to $+3.4$ logit\\
