...
The first pillar delivered a \emph{map}: every one of the $9{,}013$ patients placed on nine validated
dimensions --- severity, two kinds of biology (metabolic, inflammatory), cognition, sleep, mania, suicidality,
developmental adversity, substance --- with diagnosis deliberately held out. A map is only useful if a clinician
can \emph{read} it, and this is the second pillar's job. It begins with the question the whole ``precision
psychiatry'' field chases: do patients fall into a handful of natural \emph{types} --- biotypes --- that cut
across the old diagnostic labels? The discipline here is to \emph{not assume the answer}. The answer turns out
to be \textbf{no}, and that negative is what makes the rest of the pillar honest: rather than a typology, M2
delivers a \textbf{coordinate system} for placing any patient and --- the value this pillar is built on --- an
\textbf{archetype reading guide} that turns a nine-number coordinate into a small set of clinically nameable
extremes a clinician can actually use, without ever pretending the boxes are real.
...
\subsection{The archetype simplex: every patient a blend of extremes}
\label{sub:simplex}
A continuum has no clusters, but it still has a \emph{shape}, and the shape has \textbf{corners} --- extreme
phenotypes the cloud stretches toward. \emph{Archetypal analysis} recovers them: it writes every patient
$\mathbf{x}_i$ as a convex blend of $A$ extreme profiles $\mathbf{z}_a$,
\begin{equation}
\min_{\mathbf{W},\mathbf{B}}\ \sum_i\Bigl\lVert\mathbf{x}_i-\sum_{a=1}^{A}w_{ia}\mathbf{z}_a\Bigr\rVert^2
\quad\text{s.t.}\quad \mathbf{z}_a=\sum_j b_{ja}\mathbf{x}_j,\quad
w_{ia},b_{ja}\ge0,\quad \textstyle\sum_a w_{ia}=\sum_j b_{ja}=1 .
\label{eq:aa}
\end{equation}
Two constraints carry the meaning. Each corner $\mathbf{z}_a$ is itself a convex combination of \emph{real
patients} ($b_{\cdot a}$ on the simplex), so the corners sit on the \textbf{convex hull} of the cloud --- they
are genuine extremes anchored in data, not abstract directions. And each patient's weights $w_i$ live on the
\textbf{simplex} ($w_{ia}\ge0$, summing to one): with $A{=}4$ corners every patient is a point inside a
tetrahedron and $w_i$ are its barycentric coordinates --- literally how much of each corner the patient is.
This is the opposite of PCA, which threads its axes through the \emph{centre} of the cloud; archetypal analysis
reaches for the \emph{poles} (Figure~\ref{fig:simplex}). Four corners are the stable resolution
(\S\ref{sub:archrobust} confirms it: cross-seed Tucker $0.997$ at $A{=}4$, while $A{=}5$ fails to reproduce),
and because the fit runs over the M1 posterior \emph{draws}, each $w_i$ carries its own uncertainty --- the
blend has error bars too.

\begin{figure}[h]\centering
\archetypesimplexblend
\caption{The archetype simplex. The four extreme phenotypes --- \textbf{A0} biological (cardiometabolic $+$
inflammatory $+$ substance), \textbf{A1} low-burden, \textbf{A2} severe-but-low-biology, \textbf{A3}
symptom-driven --- are the corners; every patient sits \emph{inside} as a convex blend whose weights sum to one
(a worked example at the foot). The load-bearing dissociation: \textbf{A0} and \textbf{A2} are \emph{separate}
corners at essentially the \emph{same} severity but opposite biology --- biology$\perp$severity, rendered as a
direction you can point to.}
\label{fig:simplex}
\end{figure}

\begin{keyresult}[What the simplex buys you that a partition cannot]
The corners make the transdiagnostic dissociation \emph{pointable} (A0 vs.\ A2, \S\ref{sub:reading}); they
recover \textbf{biology} --- which has no density gap and so survives no hard split --- as a first-class
\emph{extreme} (\S\ref{sub:tessellation}); they compress a nine-vector into a handful of weights a clinician can
say out loud; they are the representation the prognosis chapter finds \emph{predictive} of two-year functioning
beyond DSM-5 and severity (\emph{operative $K=$ none}, \S\ref{sec:prognosis}); and they are a \emph{fixed frame}
any new patient or follow-up visit projects onto by simplex least-squares, with no refitting.
\end{keyresult}

\subsection{Reading the map (I): four corners and their representative patients}
\label{sub:reading}
Having established the corners as the vertices of the simplex (\S\ref{sub:simplex}), we now \emph{name} them.
Because a pure corner is an extreme almost
...
Each patient is a Gaussian on the map, $\Normal(\mu_i,\,\sigma_i^2)$. We score similarity by the
uncertainty-weighted distance introduced with the coordinate object (Eq.~\eqref{eq:uwdist}) --- a per-dimension
standardized distance under the combined measurement uncertainty. A dimension either patient is uncertain about contributes
...
Step back from the battery and one sentence holds the chapter together. The continuous coordinates say
precisely \emph{where} each patient is, with honest error bars (\S\ref{sub:coords}); the simplex says \emph{what
blend} of four clinically meaningful extremes that position is (\S\ref{sub:simplex}) --- and because biology is
a density-gapless extreme, the simplex is the only view that keeps biology a \textbf{first-class, forecastable
axis} rather than dissolving it into a symptom-driven partition. That dissociation is what a clinician's global
impression cannot see, and it is precisely why M2 ships a coordinate system with a reading guide, not a typology.
