...
\paragraph{Are the corners real, or does the method invent them?}
Archetypal analysis \emph{always} returns $A$ corners --- so, exactly as we falsified clusters before trusting
them, we falsify the corners before leaning on them (a full battery; Figure~\ref{fig:archrobust}). The verdict
is \textbf{8 of 9 checks pass}. $A{=}4$ is the right count (cross-seed reproducibility is $\approx 1.0$ through
$A{=}4$ and \emph{collapses} to $0.51$ at $A{=}5$); the corners reproduce across seeds (Tucker $0.997$),
independent halves ($0.94$), and patient bootstraps ($0.95$), are not duplicates (pairwise cosine $-0.11$), and
the \emph{native} $A{=}8$ does not transfer to the copula ($0.32$) --- confirming it was a native-map artefact.
Two results are themselves informative: a \emph{single} optimization restart is unreliable (congruence
scatters $0.36$--$0.99$), so the production protocol uses four (necessary and sufficient $\to 1.0$); and the
one conditional check is that the corner \emph{locations} under fully propagated measurement uncertainty are
only moderately precise (median Tucker $0.69$ over single posterior draws, vs $0.95$ under resampling) ---
which is precisely \emph{why} the load-bearing object is the continuous coordinates with their uncertainty, and
the corners are a reading lens rather than fixed coordinates.
