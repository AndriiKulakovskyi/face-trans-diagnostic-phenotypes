\section{Milestone 4: prognosis --- does the map predict?}
\label{sec:prognosis}

M3 showed the map and its strata \emph{persist}; M4 asks whether they \emph{predict}. The distinction is
the whole point of the program: a description can be stable and still clinically idle. The question is
sharp and deliberately conservative --- does a baseline coordinate or stratum forecast a future clinical
outcome \emph{incrementally beyond what a clinician already has}, namely the DSM-5 diagnosis, the current
severity, and the present value of the outcome itself? The bet, placed exactly where M3 pointed, is that
the \emph{durable} trait axes (cognition, metabolic, inflammatory) predict the \emph{moving} state outcomes
(functioning, severity). Throughout, M4 is a pure consumer of the fixed M1/M2/M3 objects --- the panel of
re-scored coordinates, their per-patient uncertainty, the archetype memberships, and the attrition weights
--- never re-estimated (invariant I3). The outcomes are clinical scales re-administered over the dense
yearly grid V0\,$\to$\,V2; there is no hospitalization or relapse register in the record, so M4 forecasts
...
coefficient toward zero --- worst where the coordinate is least certain. So each map predictor enters as
a latent true value with the \emph{known} M1 per-patient standard deviation plugged in (the M3
variance-model trick, Eq.~\ref{eq:traitstate}, generalized to the outcome equation):
\begin{equation}
\label{eq:eiv}
...
(the $G$-residualized Arm-B view; $+58$ for the full-phenotype Arm-A) and the four-region tessellation
$+47$ --- clearly predictive. The durable \emph{biology} coordinates carry it: metabolic
...
as well as the manifest CGI-S. This is the M1 bet --- biology orthogonal to $G$ --- cashed out
prognostically: the metabolic/inflammatory corner a patient \emph{keeps} (M3 trait) forecasts the
functional state they \emph{move toward} (M3 state). Cognition does not reach credibility (prior-dominated
for the untested, and the weakest of the three).
...
categorical illness type, the $\perp G$ map the dimensional biology profile --- so the map
...
follow-up while keeping rank --- the M3 trait/state geometry, now in outcomes.}
\label{fig:m4atlas}
\end{figure}
...
\subsection{What M4 establishes --- and what it defers}
\label{sub:m4limits}
...
\begin{table}[h]\centering\small\sffamily
\caption{The M4 gates and verdicts (internal validity, V0$\to$V2; functioning as the primary outcome).}
\label{tab:m4goals}
...
\begin{openbox}[Prediction is not moderation --- that is M5]
M4 shows the map \emph{forecasts} who will do well; it does not show it \emph{changes what to do}.
Whether a stratum \emph{moderates treatment response} --- the stratum-by-treatment interaction, the point
at which a phenotype alters management --- is M5, and it will require a treatment or randomization
variable the present record may not contain (the enrolment \texttt{arm} is a DSM-5 subtype, not an
assigned treatment). Caveats carried forward: scale trajectories, not events; internal association, not a
