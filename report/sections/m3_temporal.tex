\section{Milestone 3: temporal coherence --- does the map hold over time?}
\label{sec:temporal}

M1 gave the map and M2 gave its strata, both discovered on the baseline visit alone. A map drawn once can
still be a snapshot of nothing: the dimensions might be an artefact of how patients happened to present on the
day they enrolled, the strata a momentary arrangement that dissolves by the next visit. M3 asks the question
that makes the first two milestones mean something --- does the signal \emph{cohere and persist} over
...
do the phenotype memberships persist? Throughout, the follow-up visits are scored onto the \emph{fixed} M1/M2
model --- observed cells only, uncertainty propagated, the map never re-estimated on later data (invariant
I3). The window is the dense yearly grid V0\,$\to$\,V1\,$\to$\,V2, where all three cohorts are well represented
($9{,}013 \to 4{,}270 \to 2{,}958$ patients).

\subsection{A geometrically derived, falsifiable prediction}
M3 is not a fishing expedition. The M2 geometry makes a specific prediction (Figure~\ref{fig:temporalspine}):
...
makes M3 a test rather than a restatement.

\begin{figure}[h]\centering
\temporalspine
\caption{The M3 prediction, made by the M2 geometry. Over follow-up the cohort \emph{slides} along the severity
spine (the population improves --- a state dynamic), while each patient's biological \emph{corner} stays fixed
(a trait). The milestone tests this two independent ways: a variance decomposition (G3) and a per-patient
...
the same way --- otherwise apparent movement is measurement drift. So before interpreting anything, M3 refits
...
M1's cross-cohort invariance test, with cohort swapped for visit.
...
within-patient \emph{state} variance, with the M1 measurement error \emph{plugged in} rather than estimated. The
...
\emph{known} M1 posterior variance. The trait ratio is the intraclass correlation $\mathrm{ICC}_d =
...
what forces the milestone's central nuance.
\end{methodsbox}
...
\label{sub:synthesis}
The milestone's real test is whether the variance route (G3) and the geometry route (G4) \emph{agree}. The
...
the core while diverging for \emph{named} reasons is the stronger evidence. The M2 geometry --- severity the
spine, biology the corners --- is temporally coherent.
\end{keyresult}

\subsection{Honesty: is the retained sample fair? (G6)}
Attrition is the temporal selection threat --- the analogue of M1's completeness concern. If the sicker
...
\subsection{What M3 establishes --- and what it defers}
M3 takes the baseline map and strata and shows they are \emph{temporally coherent}: the measurement holds
across follow-up (G1), and the M2 geometry replays --- the cohort slides on severity and symptoms while
...
\begin{table}[h]\centering\small\sffamily
\caption{The M3 goals and verdicts (internal validity, V0$\to$V2). G5 --- a temporal head-to-head against the
diagnostic labels --- is absent by data design: the enrolment diagnosis is time-invariant in the record, so it
is deferred to M4 with the diagnosis-change exit signal captured for it.}
\label{tab:m3goals}
...
G2 substrate & the longitudinal panel & built; V0 reproduces M2 at $99.9\%$\\
...
\begin{openbox}[Persistence is not prediction --- that is M4]
M3 establishes that the signal \emph{coheres and persists}; it makes no prognostic, treatment, or external
claim. Whether a baseline coordinate or stratum \emph{predicts} a future outcome --- incrementally beyond
diagnosis and severity --- is M4, and the durable axes M3 identified (cognition, metabolic, inflammatory) are
