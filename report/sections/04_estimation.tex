...
$71\times 71$. Correlated factors fold in through the reparameterization $\widetilde\Lambda=\Lambda\,\mathrm{chol}(\Phi)$,
...
loading/residual parameters --- the $\sim$70{,}000 patient latents are gone.
...
A third lever is the Gaussian copula itself. Because the copula tiering (\S\ref{sub:copula}) promotes the
high-resolution ordinal and count indicators into the analytically-marginalized Gaussian block --- indicators
that under the native mixed likelihood would each carry a \emph{sampled} per-patient latent --- it
\emph{shrinks} the explicit-latent block $f_e$ that is the sole remaining mixing bottleneck. That is a proximate
reason the cohort-weighted joint fit finally clears sub-$1.05$ $\widehat R$: the nine-dimension joint mixes at
$\widehat R\le1.04$ at the largest $N$ that mixes, with the continuous backbone certified at full $N$
($\widehat R=1.01$). Crucially, the copula \emph{expanded} the reach of analytic marginalization; it did not
\emph{create} it --- the Gaussian latents were always integrated out (Eq.~\ref{eq:marginal}), and the copula
merely moved more indicators under that same exact integral.
