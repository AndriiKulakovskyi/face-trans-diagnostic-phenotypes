...
\begin{tikzpicture}[>={Stealth[length=2.6mm]},node distance=5mm,
  ...
  \node[stp,right=of s4] (s5) {\bfseries S5\\\scriptsize all dimensions\\\scriptsize $+$ correlated-$G$\\[2pt]\scriptsize \bfseries the reported map};
...
warm-starts the next. Green $=$ certified at full $N$; amber $=$ the reported global fit, provisional at
sub-sample. \emph{Only $\mathcal{M}_5$ is interpreted.}}
\label{fig:staging}
\end{figure}
...
and a natural QC/discussion gate after each step.
...
For continuous indicators the latent factors can be integrated out analytically. Stacking the continuous
...
(Section~\ref{sec:validation}), and (ii) this marginal is also the computational route that removes the
...
($\Psi$ diagonal; $\Lambda\Lambda\Tn$ rank $F=K{+}1$), so the Woodbury identity and the matrix-determinant
lemma move the expensive work into factor space. With $W=\Psi^{-1}$ (the observed-cell precision; $W_{ij}=0$
...
\rowcolor{facebg} warmup length & staged warm-starts (continuation, \S\ref{sec:estimation}) & each stage starts in the previous basin; shorter tuning, fewer failed trajectories\\
