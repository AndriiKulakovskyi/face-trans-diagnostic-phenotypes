...
certified at the largest $N$ that mixes (cross-seed Tucker $\varphi$ 0.993). \emph{Only $\mathcal{M}_5$ is
...
Section~\ref{sec:engineering}). The explicit-latent block remains the documented mixing limit, so the joint map
is certified at the \emph{largest $N$ that mixes} with a cross-seed resample-stability guard.
\end{methodsbox}
