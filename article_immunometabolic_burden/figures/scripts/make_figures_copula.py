...
# Re-point from the legacy results/face/*_oop layout to the clean milestone roots.
# Call sites still pass the old first segment (e.g. "strata_oop"); we remap it here.
_MILESTONE = {
    "strata_oop": ("m2_strata",),
    "temporal_oop": ("m3_temporal",),
    "prognosis_oop": ("m4_prognosis",),
    "treatment_oop": ("m5_treatment",),
    "m4_prognosis/repbench": ("m4_prognosis", "repbench"),  # repbench: not regenerated by M1-M5
}
def R(*p):
    p = list(p)
    if p and p[0] in _MILESTONE:
        p = list(_MILESTONE[p[0]]) + p[1:]
