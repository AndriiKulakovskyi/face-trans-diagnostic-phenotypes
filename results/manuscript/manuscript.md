...
abstract: |
  ...
  **Results.** Trans-diagnostic variation was **dimensional, not categorical**: three reproducible axes — **internalizing, cognition, cardiometabolic–inflammatory** — with **no dominant general factor** (ECV = 0.42) and **no discrete subtypes** beyond the DSM cohorts themselves. (A fourth, *illness-course*, was the least robust dimension and did not survive a deliberate stress-test: adding an orthogonal lifetime substance-use-disorder construct collapsed its split-half reproducibility, so we report the conservative three-axis lock.) The axes were not a cohort artifact (cohort-residualized congruence ≥ 0.96) and were granularity-invariant (top-two canonical r = 0.99/0.90). The headline result: **symptoms and biology are nearly orthogonal** (between-block mean |r| = 0.03), and the general factor is a **symptom-only artifact** — its first-factor share falls monotonically from **0.33 (symptoms only)** to **0.09 (integrated)** as *structured* biology and cognition are admitted. The structure was longitudinally coherent (cardiometabolic the most trait-stable axis). Against DSM, the dimensions added a **modest but real** prognostic increment — robustly for functioning (GAF ΔR² = +0.044; FAST +0.041) and, once a regression-to-the-mean confound was removed, modestly for relapse (de-confounded ΔAUC = +0.036 by logistic regression; early-course prognosis reached AUC ≈ 0.70).
...
A fourth dimension — **illness-course** (later age at first hospitalization/treatment/episode against inverse lifetime hospitalization burden) — emerges under a four-factor solution but proved the **least robust** axis. When an orthogonal lifetime **substance-use-disorder** construct (alcohol/cannabis abuse-or-dependence; itself uncorrelated with every axis, $|\text{loading}| \le 0.07$) was admitted to the second-order extraction, the four-factor split-half congruence **collapsed from 0.96 to 0.31** while the three-factor solution stayed reproducible (0.92) — so the data lock moved to **K = 3** ("first collapse minus one"). The illness-course content did not disappear so much as **disperse**: its inverse-burden term re-surfaces weakly on the cardiometabolic axis (chronic illness co-occurring with metabolic load), while its age-of-onset core no longer anchors a standalone factor (max |loading| 0.29). We therefore report the conservative, reproducible three-axis structure and treat illness-course as a fragile candidate dimension rather than a headline result.

The axes are only weakly correlated (mean $|\Phi_2| = 0.12$; the largest is internalizing↔cardiometabolic $+0.25$, reflecting that worse mood co-occurs with higher cardiometabolic load). Critically, the Schmid–Leiman test returned **ECV = 0.42** ($< 0.5$) — **no dominant general (*p*-)factor**; the structure is genuinely multidimensional. Three well-measured constructs — **mania/activation** (Altman + YMRS, $\mathrm{VAF}_1=0.71$), **suicidal ideation** (ISF), and **substance-use disorder** (alcohol/cannabis, $\mathrm{VAF}_1=0.86$) — proved **orthogonal** to the axes (max $|\text{loading}|$: mania 0.07, substance 0.07, suicidal ideation 0.25 — all below the 0.30 axis-membership threshold) and are reported as independent standalone dimensions rather than forced into the correlated structure — itself a finding (mania's independence from internalizing; addiction's independence from every axis). Notably, suicidal ideation remained orthogonal *even after* skip-logic decoding recovered its attempt-count items (§2.3; coverage 25–38% → 72–92%), so its standalone status is a structural fact, not a missing-data artifact.
...
We foreground, rather than bury, the design limits. **Internalizing is BP + DR-anchored**: its defining mood/anxiety/functioning scales are absent in FACE-SZ by protocol, so schizophrenia is scored by proxy and the "trans-diagnostic" claim is qualified for that axis alone. **Cognition is baseline-anchored** (the WAIS battery is largely not re-administered), so its longitudinal test is limited to V2. A fourth, **illness-course** axis (age-of-onset against inverse hospitalization burden) emerged at K = 4 but was the least robust dimension: a deliberate stress-test — admitting an orthogonal lifetime substance-use-disorder construct — collapsed its split-half reproducibility (0.96 → 0.31), moving the conservative data lock to K = 3. We therefore do not report it as an axis; its inverse-burden content re-surfaces weakly on the cardiometabolic axis. **Cardiometabolic is the most trait-stable, symptom-orthogonal** axis with a three-cohort biological core; its one weakness is the small within-cohort tests (within-SZ 0.35, within-DR 0.78, n = 552, underpowered), although it survives cohort-residualization (0.98). The depression cohort is small (n = 552) and collapses longitudinally, so all temporal and predictive claims are effectively bipolar + schizophrenia; reassuringly, leave-depression-out congruence was 0.97, so the structure does not depend on it.
