...
abstract: |
  ...
  **Methods.** We harmonized three deeply-phenotyped FACE cohorts — bipolar disorder (n = 6,252), schizophrenia (n = 2,209) and major depression (n = 552; total **N = 9,013**) — into a 201-variable common dictionary spanning symptoms, biology (labs, vitals/ECG) and cognition (WAIS/TMT, verbal memory, verbal fluency). Under a strict **no-imputation** rule, all structure was estimated from a masked pairwise-complete correlation with regression (Thomson) factor scores computed on each patient's observed support only. We fitted a **hierarchical/bifactor measurement model** (196 baseline items → 95 first-order constructs → second-order trans-diagnostic dimensions), tested a general factor by Schmid–Leiman explained common variance (ECV), and locked dimensionality by split-half Tucker congruence. A parallel masked-similarity stratification arm tested for discrete subtypes. Four pre-registered validation studies probed cohort confounding (A), symptom–biology orthogonality and the *p*-factor (B), longitudinal coherence (C), and incremental prediction over DSM (D).

  **Results.** Trans-diagnostic variation was **dimensional, not categorical**: three weakly-correlated trans-diagnostic axes — **internalizing, cognition, cardiometabolic–inflammatory** — with **no dominant general factor** (ECV = 0.42), alongside several reproducible but **mutually-independent standalone dimensions** (illness-course, substance-use, mania, suicidality), and **no discrete subtypes** beyond the DSM cohorts. A 50-replicate cohort-stratified bootstrap clarified the structure: the individual factors are highly stable (each recovering in 98–100 % of resamples), while the *number* of correlated axes is robustly three (beyond which the eigenvalues are near-degenerate, so the additional reproducible dimensions are independent of the backbone and of each other). The axes were not a cohort artifact (cohort-residualized congruence ≥ 0.96) and were granularity-invariant (top-two canonical r = 0.99/0.90). The headline result: **symptoms and biology are nearly orthogonal** (between-block mean |r| = 0.03), and the general factor is a **symptom-only artifact** — its first-factor share falls monotonically from **0.33 (symptoms only)** to **0.09 (integrated)** as *structured* biology and cognition are admitted. The structure was longitudinally coherent (cardiometabolic the most trait-stable axis). Against DSM, the dimensions added a **modest but real** prognostic increment — robustly for functioning (GAF $\Delta R^2$ = +0.044; FAST +0.041) and, once a regression-to-the-mean confound was removed, modestly for relapse (de-confounded ΔAUC = +0.036 by logistic regression; early-course prognosis reached AUC ≈ 0.70).
...
|---|---:|---:|---:|---:|
| item (183) | 3,453 | 10.1% | 0.6% | ≈ 1.4 × $10^{9}$ |
| construct (72) | 4,102 | 3.4% | 0.0% | ≈ 1.1 × $10^{2}$ |
...
averaged over splits. Because split-half congruence is non-monotonic in $K$ (a known trap — naïve "max-$K$ above threshold" rules recover spurious high-$K$ solutions with improper Heywood loadings), we lock $K$ at the **first collapse minus one** and confirm by a **per-factor** congruence refinement (a single unstable factor can drag the minimum down without indicting the stable ones). Because this scalar $K$ proved sensitive to the construct set, we additionally quantified its stability — and, separately, the stability of the *individual* factors — by a 50-replicate **cohort-stratified bootstrap** (eigengap 95 % CIs of $\Phi_1$; the distribution of the locked $K$; and per-factor Tucker congruence at a fixed $K=6$ against the full-sample solution; `scripts/sensitivity_bootstrap_dimensionality.py`, reported in §3.1).
...
(with $g_1,g_2$ the sample skewness and excess kurtosis); bootstrap cluster stability; and agreement with DSM labels by the adjusted Rand index (ARI). Two feature sets were used: **A**, the 3 dimensions plus the 2 orthogonal standalone constructs (mania, suicidality); **B**, the 82 construct scores.
...
## A three-dimension trans-diagnostic structure, with no general factor

The second-order model resolved **three reproducible, weakly-correlated trans-diagnostic axes** — the correlated *backbone* of the structure (**Fig. 2**):

1. **Internalizing** — depression (QIDS 0.93, MADRS 0.86), anxiety (STAI 0.80), anhedonia (QIDS-13 loss of interest, 0.72), disability (FAST 0.72) and poor functioning/quality-of-life (GAF, EQ-5D, reverse-keyed);
2. **Cognition** — anchored by **verbal episodic memory** (CVLT total recall 0.77, delayed free recall 0.75), with verbal fluency (semantic 0.64), executive function (TMT-B 0.68), processing speed (0.66) and psychomotor speed co-loading, and education a negative correlate;
3. **Cardiometabolic–inflammatory** — inflammation (CRP/WBC, 0.47), adiposity (0.44), HDL/triglycerides (0.42), autonomic heart rate (0.41), lymphocytes, blood pressure and hepatic enzymes; it also carries a residual inverse-chronicity signal (lifetime hospitalization burden, 0.34).

These three axes are the correlated **backbone** — but they are not the only reproducible structure, and the *number* of correlated axes, unlike the axes themselves, is not sharply determined. A 50-replicate cohort-stratified bootstrap makes the distinction precise (**Fig. S1**). The *individual* factors are highly stable: every multi-construct factor — the three axes plus **illness-course** (age-of-onset/chronicity), **substance-use** and a **childhood-adversity** factor — recovers in **98–100 %** of resamples (mean Tucker congruence ≥ 0.97). The *count* of correlated axes, by contrast, is a noisy estimator (the split-half rule returns **K = 3 in 60 %** of resamples, K = 2 in 26 %, K ≥ 5 in 14 %), because beyond the three dominant directions the eigenvalues of $\Phi_1$ form a **near-degenerate cluster**: the first two gaps are large (2.9 and 2.2, 95 % CIs excluding 0) but $\lambda_4-\lambda_5 = 0.11$ (95 % CI [0.02, 0.21]) is statistically indistinguishable from zero, so a "fourth axis" is not individually identified and admitting any comparable small construct (here substance-use) merely re-orders it. The extra factors are therefore **real but mutually orthogonal**, and orthogonal to the backbone — illness-course carries only a weak 0.3 residual onto cardiometabolic and anchors no correlated axis (max |loading| 0.29). We thus report a **three-dimensional correlated backbone plus several independent standalone dimensions**, rather than forcing a single integer onto a structure that is not one correlated space — a description that, if anything, *reinforces* the central result: the more mutually-independent dimensions the data holds, the less any single general factor can span it.

The three backbone axes are themselves only weakly correlated (mean $|\Phi_2| = 0.12$; the largest is internalizing↔cardiometabolic $+0.25$, reflecting that worse mood co-occurs with higher cardiometabolic load). Critically, the Schmid–Leiman test on the backbone returned **ECV = 0.42** ($< 0.5$) — **no dominant general (*p*-)factor** — and the standalone dimensions add still more *independent* variance, so the integrated structure is emphatically multidimensional. The standalones are well-measured dimensions in their own right — **mania/activation** (Altman + YMRS, $\mathrm{VAF}_1=0.71$), **suicidal ideation** (ISF), **substance-use disorder** (alcohol/cannabis, $\mathrm{VAF}_1=0.86$) and **illness-course** — with maximum loadings on any axis of 0.07, 0.25, 0.07 and 0.29 respectively, all below the 0.30 axis-membership threshold. That mania is independent of internalizing, and addiction of every axis, are themselves findings; and suicidal ideation remained orthogonal *even after* skip-logic decoding recovered its attempt-count items (§2.3; coverage 25–38% → 72–92%), so its standalone status is structural, not a missing-data artifact.

The three-dimension solution passed every validation check (**Table 2**): it was confound-clean (no axis explained >0.25 by cohort, sex, age, site or per-patient missingness; cognition's cohort $\eta^2$ is matched by an education $\eta^2$, a genuine correlate); trans-diagnostic yet clinically valid (internalizing highest in depression, cognition worst in schizophrenia); **leave-cohort-out reproducible** for the well-measured axes (dropping BP / SZ / DR gave minimum per-axis congruence 0.66 / 0.91 / 0.97 — the drop-BP minimum falls on the cardiometabolic axis, which is only weakly measured in the SZ + DR remainder); and **granularity-invariant** — canonical correlations between the hierarchical axes and a flat-domain factor solution were **0.99, 0.90, 0.07** (permutation null ≈ 0.04), so the top two axes are *not* an artifact of how items were grouped (the third differs only because the hierarchical model adds the recovered labs/vitals the flat domains dropped). A polychoric (tetrachoric) sensitivity reproduced the three dimensions exactly (congruence 1.00) — including the binary-derived substance-use construct (Pearson↔tetrachoric score r = 0.999) — and suicidal ideation remained sub-threshold even after skip-logic recovered its coverage, confirming its exclusion is neither a Pearson-attenuation nor a missing-data artifact.

| Axis | Max confound η²/R² | Cohort-residualized congruence | Granularity *r* (hierarchical vs flat) | Bootstrap stability |
|---|---:|---:|---:|---:|
| Internalizing | 0.12 (DSM arm) | 0.99 | 0.99 | 100% |
| Cognition | 0.18 (education) | 0.98 | 0.90 | 100% |
| Cardiometabolic | 0.02 | 0.98 | 0.07\* | 100% |

Table: **Validation of the three trans-diagnostic axes (Stage 4).** No axis is a confound marker (every η²/R² < 0.25 against cohort, DSM arm, sex, age, education, site and per-patient missingness); cognition's cohort η² = 0.16 is matched by an education η² = 0.18, a genuine correlate. Cohort-residualized re-derivation (Study A) and the 50-replicate bootstrap reproduce every axis. \*Cardiometabolic's low granularity *r* reflects the hierarchical model recovering labs/vitals that the flat domains dropped, not an artifact. Leave-cohort-out: dropping BP / SZ / DR gave minimum per-axis congruence 0.66 / 0.91 / 0.97; a polychoric (tetrachoric) sensitivity reproduced all three axes (congruence 1.00).

![**Figure 1. Study design and analytic pipeline.** Three FACE cohorts (N = 9,013) are harmonized into a 201-variable dictionary and processed in three stages (native scale → type-aware scaling to [-1,1] → V0 item matrix) under a strict no-imputation rule. A hierarchical/bifactor measurement model maps 196 baseline items → 95 first-order constructs (within-construct masked one-factor posteriors) → three second-order trans-diagnostic dimensions plus orthogonal standalone dimensions (illness-course, substance-use, mania, suicidality; ECV 0.42 → no *p*-factor). Two analysis arms (dimensional; stratification) feed four validation studies (A–D).](../reports/figures/fig1_pipeline.png){width=6.6in}

![**Figure 2. The three trans-diagnostic dimensions.** Defining constructs per axis (|second-order loading| > 0.30; red = higher score more pathological, blue = reverse-keyed). The $\Phi_2$ panel shows weak inter-axis correlations (mean |$\Phi_2$| = 0.12); Schmid–Leiman ECV = 0.42 indicates no dominant general factor. Illness-course, substance-use, mania and suicidal ideation are valid, reproducible dimensions but are orthogonal *standalones*, not part of the correlated backbone (max axis loading ≤ 0.29). Bootstrap robustness of the structure is shown in Fig. S1.](../reports/figures/fig2_axes.png){width=6.3in}

## Trans-diagnostic variation is dimensional, not categorical

Both analysis arms agreed that the structure is **continuous, not categorical** (**Fig. 4**). On the dimensions (set A), HDBSCAN found **no real density structure** (only two micro-pockets at 82% noise, themselves unrelated to cohort, ARI 0.04); the real silhouette barely exceeded a Gaussian-null silhouette and did not peak at any $k$ (gaps 0.01–0.05); every axis was unimodal (Sarle $b < 0.50$); and $k$-means partitions bore essentially no relation to DSM (ARI ≈ 0.04). On the 82 construct scores (set B), the **only** dense clusters HDBSCAN recovered were the **three cohorts themselves** (ARI = 1.00) — i.e. the sole categorical structure in the data is the DSM diagnosis, and finer granularity revealed **no novel subtypes**. (High $k$-means bootstrap stability, 0.79–0.93, is a continuum artifact — $k$-means partitions a blob reproducibly — and is over-ruled by the absence of density clusters and the unimodality.) Combined with the absence of a general factor, this yields a clean dimensional account: **continuous axes, no *p*-factor, no discrete subtypes**.
...
Study A refuted the obvious confound and, in doing so, produced an important qualification. Re-deriving the structure after **residualizing every construct on cohort** (removing all between-cohort means) reproduced all three axes with congruence ≥ 0.98; within bipolar disorder alone (n = 6,252) all three reproduced ≥ 0.95. The axes are therefore **within-cohort covariance**, not between-cohort batch effects.

The within-cohort analyses revealed *why* two axes are weaker in places, and it is largely a **measurement-coverage** fact, not a clinical subtlety. The internalizing axis's defining scales — MADRS, QIDS, STAI, FAST, Altman, PRISM, CSM — are **0% observed in FACE-SZ**, which used a psychosis battery by design; schizophrenia patients are scored on internalizing only through surviving three-cohort proxies (GAF, CGI, PSQI, EQ-5D), giving within-SZ congruence 0.57. By contrast, **cognition is cleanly three-cohort** (within-SZ 0.96, within-DR 0.91; top constructs 43–92% observed in SZ), and cardiometabolic is three-cohort in its biological core (lipids, adiposity, glycaemia, CRP) with only peripheral BP + DR additions, though its small within-cohort re-derivations are underpowered (within-SZ 0.35, within-DR 0.78, n = 552). The consequence, which we state plainly throughout: the **fully** trans-diagnostic axis is **cognition**; **cardiometabolic** is three-cohort in its biological core but reproduced only weakly in the within-SZ and small within-DR tests (robust under cohort-residualization, 0.98); and the **internalizing (mood) axis is directly measured only in BP + DR and represented by proxy in schizophrenia**. The two weakly-reproduced axes (internalizing, cardiometabolic) are flagged accordingly. The dimensional, no-*p*-factor and no-subtype results are unaffected; the "trans-diagnostic" label is qualified for internalizing (a measurement gap in SZ) and, with a power caveat, for cardiometabolic.

## Longitudinal coherence

The structure persisted at follow-up wherever it could be re-measured (**Fig. 6**, Study C). Re-estimated per visit, the loadings were highly congruent with V0 for internalizing (0.99 / 0.97 at V1 / V2) and cardiometabolic (0.92 / 0.95), with cognition recovering at V2 (0.95; lower at V1, 0.58, because the WAIS battery is baseline-anchored and only ~5% re-measured at V1). Rank-order score stability separated **trait** from **state**: cardiometabolic was the most trait-stable (Spearman 0.61 / 0.56 at V0↔V1 / V0↔V2), internalizing was moderately stable as befits episodic mood (0.58 / 0.53), and cognition was lowest at V1 (0.30 — a coverage artifact of the baseline-anchored battery) recovering by V2 (0.50). Together with Study B, this identifies the **most measurement-robust trans-diagnostic substrate as biological**, with symptom structure more state- and cohort-specific.

![**Figure 6. Longitudinal coherence (V0 → V1 → V2).** (a) Structural invariance: per-visit loadings are highly congruent with V0 for internalizing and cardiometabolic; cognition recovers at V2 (its battery is baseline-anchored, ≈5% re-measured at V1). (b) Score test–retest separates trait (cardiometabolic) from state/episodic (internalizing) axes; cognition's low V0↔V1 value is a coverage artifact of its baseline-anchored battery.](../reports/figures/fig6_longitudinal.png){width=6.6in}
...
For **functioning**, the dimensions added a modest but robust, **non-circular** increment over DSM: for GAF at V2 (n = 2,043) $\Delta R^2 = +0.044$ (95% CI +0.028, +0.059) over DSM, +0.028 cross-domain; for FAST disability (BP + DR, n = 1,878) the dimensions **beat** DSM outright ($\Delta R^2 = +0.041$ over DSM, which added nothing), +0.029 cross-domain. The functional signal was led by internalizing and cardiometabolic; cognition added little individually.

For **relapse**, the honest story required removing a confound. A change-based relapse appeared near-perfectly predicted by baseline severity (AUC 0.765) with no room for the dimensions — but that was **regression to the mean**: under the de-confounded remission-based discrete-time survival model, baseline-only prediction fell to AUC **0.579**. There the dimensions **did** add a modest increment over DSM (logistic $\Delta\text{AUC} = +0.036$, CI +0.015, +0.058; gradient boosting +0.019, ns — i.e. linearly detectable but not boosting-robust), carried by residual internalizing symptoms. Relapse-from-remission remained hard to predict (best AUC ≤ 0.65), and enriching the baseline with all 82 constructs did not break 0.64. AUC ≈ 0.70 was reachable only by adding the **early course** (predicting the V1→V2 interval from V0 + V1 trajectory, with V1 severity controlled and no leakage): there the trajectory model reached AUC ≈ 0.70 and beat DSM by +0.045 — a different, clinically sensible question (early-response prognosis), honestly distinguished from baseline prediction.
...
We foreground, rather than bury, the design limits. **Internalizing is BP + DR-anchored**: its defining mood/anxiety/functioning scales are absent in FACE-SZ by protocol, so schizophrenia is scored by proxy and the "trans-diagnostic" claim is qualified for that axis alone. **Cognition is baseline-anchored** (the WAIS battery is largely not re-administered), so its longitudinal test is limited to V2. **Illness-course** (age-of-onset against inverse hospitalization burden) is a *reproducible* dimension (bootstrap recovery 100 %) but an **orthogonal standalone**, not a fourth correlated axis: it is statistically interchangeable with the equally-reproducible substance-use factor at the near-degenerate fourth eigenvalue, so the *correlated* backbone is three-dimensional while illness-course is reported, like mania and suicidality, as an independent dimension. It is also fixed-historical (baseline-only age-of-onset) and only ~half-covered in FACE-DR. **Cardiometabolic is the most trait-stable, symptom-orthogonal** axis with a three-cohort biological core; its one weakness is the small within-cohort tests (within-SZ 0.35, within-DR 0.78, n = 552, underpowered), although it survives cohort-residualization (0.98). The depression cohort is small (n = 552) and collapses longitudinally, so all temporal and predictive claims are effectively bipolar + schizophrenia; reassuringly, leave-depression-out congruence was 0.97, so the structure does not depend on it.
...
1. *"This merely confirms HiTOP/RDoC."* The dimensional structure is partly confirmatory; the **non-derivative** contribution is the integrated-model result — symptoms $\perp$ biology and the *p*-factor as a symptom-only artifact — which symptom-only designs cannot reach.
2. *"The three cohorts are the three diagnoses, so the axes are batch effects."* Refuted in Study A: cohort-residualized re-derivation reproduces all three axes ≥ 0.98, and within-bipolar ≥ 0.95. The structure is within-cohort covariance.
3. *"Aggregating items into constructs manufactures the structure."* Granularity invariance (top-two canonical r = 0.99/0.90 vs flat-domain and item-level solutions; permutation null 0.04) shows the headline axes do not depend on grouping.
...
5. *"You over-claim 'trans-diagnostic' when internalizing isn't measured in schizophrenia."* We state this prominently and restrict the fully trans-diagnostic claim to cognition and core cardiometabolic.
6. *"No imputation with masked correlations is unstable and you're hiding non-PSD matrices."* We report conditioning explicitly (item κ ≈ 1.4×$10^{9}$ → construct κ ≈ 110; construct-level negative-eigenvalue mass 0%), repair to the nearest PD matrix (Eq. 4), and show a polychoric sensitivity reproduces the axes (congruence 1.00).
...
8. *"The prognostic gains are clinically trivial."* We agree they are modest and report them as such ($\Delta R^2$ ≈ 0.04), without overselling; the contribution is honesty about a small, non-circular, de-confounded increment, not a claim of clinical transformation.
...
12. *"Mania should be an axis (as in many models)."* Mania/activation is a valid, well-measured construct, but it is **orthogonal** to the three correlated axes ($|\text{loading}| \le 0.07$, robust to polychoric estimation); forcing it — or the equally-orthogonal suicidal-ideation and substance-use constructs — into the structure would misrepresent the data. Mania's independence from internalizing, and addiction's from every axis, are themselves reported.
13. *"Adding two variables changed your headline dimensionality — is K robust?"* The **factors** are robust; the **scalar count** is not; and we report the former (**Fig. S1**). A 50-replicate cohort-stratified bootstrap recovers every individual factor in 98–100 % of resamples, but the split-half $K$ is a noisy estimator (mode 3) because beyond the three dominant directions the eigenvalues are near-degenerate ($\lambda_4-\lambda_5 \approx 0$, 95 % CI [0.02, 0.21]) — a well-understood regime in which individual eigenvectors are not identified even when the subspace is. We therefore report a three-axis *correlated backbone* plus orthogonal *standalone* dimensions rather than a single integer; the backbone, the no-*p*-factor result and the orthogonality headline are all invariant to this choice.
...
![**Figure S1. Bootstrap robustness of the second-order dimensionality** (50 cohort-stratified resamples). (a) Eigenvalue gaps of $\Phi_1$ with 95% CIs: the first two gaps are large and bounded away from zero, but $\lambda_4-\lambda_5$ ≈ 0 (95% CI [0.02, 0.21]) is a *degenerate eigenpair*, so a fourth **correlated** axis is not individually identified. (b) The locked K (split-half "first-collapse-minus-1" rule) is a **noisy estimator** (K=3 in 60% of resamples; K=2 26%; K≥5 14%). (c) Yet **every individual factor** — the three axes plus illness-course, substance-use and childhood-adversity — recovers in **98–100%** of resamples. Together: the *factors* are robust, the *count* of correlated axes is not, and the data is best described as a three-axis correlated backbone plus orthogonal standalone dimensions. Source: `scripts/sensitivity_bootstrap_dimensionality.py`.](../reports/figures/figS1_bootstrap.png){width=6.8in}

The named factors are released as a documented, coverage-gated **predictive-feature atlas** (`docs/PHENOTYPE_ATLAS.md`; `scripts/export_phenotype_features.py`): the three axes plus the orthogonal standalones, each with its per-cohort coverage, temporal character (trait / state / fixed-historical) and direction, for downstream prognostic modelling.
