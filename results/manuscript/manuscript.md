...
The second-order model resolved **three reproducible trans-diagnostic dimensions** (**Fig. 2**):
...
![**Figure 2. The three trans-diagnostic dimensions.** Defining constructs per axis (|second-order loading| > 0.30; red = higher score more pathological, blue = reverse-keyed). The Φ₂ panel shows weak inter-axis correlations (mean |Φ₂| = 0.12); Schmid–Leiman ECV = 0.42 indicates no dominant general factor. Illness-course, substance-use, mania and suicidal ideation are valid, reproducible dimensions but are orthogonal *standalones*, not part of the correlated backbone (max axis loading ≤ 0.29). Bootstrap robustness of the structure is shown in Fig. S1.](../reports/figures/fig2_axes.png){width=6.3in}
...
6. *"No imputation with masked correlations is unstable and you're hiding non-PSD matrices."* We report conditioning explicitly (item κ ≈ 1.4×10⁹ → construct κ ≈ 110; construct-level negative-eigenvalue mass 0%), repair to the nearest PD matrix (Eq. 4), and show a polychoric sensitivity reproduces the axes (congruence 1.00).
