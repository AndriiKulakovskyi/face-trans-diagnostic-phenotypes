...
> **Canonical M3 findings record (8-factor map, 2026-06-27; pending PI sign-off).** Reworks the FACE M3
> temporal-coherence layer on the **8-factor** copula M1/M2 objects (continuum: continuous coordinates +
> **A = 5** archetype simplex; immunometabolic merge + substance pinned orthogonal). Engine
> [`src/face/temporal/temporal_model_oop.py`](../src/face/temporal/temporal_model_oop.py) (wraps the proven
...
object, **V1/V2 scored under the fixed copula M1**, then the established temporal kernels. No re-discovery on
follow-up. **The story replays — and the merge improves one thing (G1).**
...
**The merge resolves the native map's weak spot:** inflammatory was only *partial* on the native map; folded
into immunometabolic it is now **fully invariant (0.987)**. The biology construct is measured consistently over
...
**The durable biology is trait and is the single most durable axis (immunometabolic ICC 0.91 — identical to the
native metabolic 0.91), with cognition (0.70) trait beside it; the moving symptom axes (developmental,
suicidality, sleep) are state/mixed.** Severity is **trait *by rank*** (ICC 0.62 — patients keep their relative
position) **while the population improves** (pop_slide −0.46; suicidality slides hardest, −0.84). Clinical logic
unchanged: *stratify on the durable biology, monitor the moving symptoms.*
...
* **Archetype weights persist** — Arm-B (⊥G) weight-vector cosine median **0.81** (q10 0.49). Dominant-archetype
  agreement is **0.40** (κ 0.19) — *more argmax churn than the A = 4 native (0.49)*, which is **expected**: five
...
**Honest difference from native:** the G3⟷G4 *cross-route synthesis* (Spearman of per-axis reliable-change-rate
vs ICC) is **ρ = 0.07 (p = 0.87)** — near zero, where the native was a weak-but-right −0.27. The two routes no
longer clearly co-rank on the 8-factor map (the merge + A = 5 churn + the data-limited mania/orthogonal
substance muddy a 8-point correlation). G3 (the error-corrected ICC) remains the clean headline; read G4 as
geometric persistence of identity (cosine 0.81), not as an independent confirmation of the variance ranks.
...
* **Substance ICC dropped vs the 9-factor map** (0.49 mixed, was 0.88 trait). Expected: substance is now pinned
  **orthogonal** (no Φ borrowing) and is thin (2 SUD binaries) — its coordinate is noisier, so less of its
  variance reads as durable trait. Honest consequence of the cleaner parameterization, not a regression.
* **Mania is uninformative** (ICC high but signal-ratio < 0.5 — 2 indicators); its verdict is not load-bearing.
* **Developmental scores state** — as on the native map this partly reflects CTQ recall-noise (the construct is
  trait by design); interpret with care.
* **V0 reused from the M2 posterior, V1/V2 projected** (the native asymmetry); internal/temporal-coherence
  validity only.
...
(4/4 invariant, immunometabolic now fully invariant), biology is durable (immunometabolic ICC 0.91) while
symptoms slide, and archetype identity persists in the weights (cosine 0.81) — reproducing the native M3
headline on the better map, with the honest caveats above (softer G3⟷G4 cross-route agreement; substance now
orthogonal).
