...
> precision-psychiatry plan ([`../V3_PLAN.md`](../V3_PLAN.md) · [`../ROADMAP.md`](../ROADMAP.md)). Every
> result below is a **V3 hypothesis to confirm / refine / downgrade** under the patient-level
> observed-likelihood model — *not* a settled conclusion. The carry-forward of each V2 finding into a
> V3 hypothesis lives in [`../FINDINGS.md`](../FINDINGS.md); folder overview in [`README.md`](README.md).
...
### First-order structure (Stages 0–2, scripts 01–32)
...
### Second-order structure (Stage 3, script 04) — PROVISIONAL (pre-validation)
...
### Validation (Stage 4, script 06) — the 4-dim solution PASSES
...
**Verdict: DIMENSIONAL (continuum), not discrete** (`scripts/07_phase5_stratify.py`). Structure test
...
- **Study A — cohort confound: axes are NOT a cohort artifact** (`scripts/09_cohort_confound.py`).
...
- **Study B — symptom⊥biology + p-factor is a symptom-only artifact (HEADLINE)**
  (`scripts/10_orthogonality_pfactor.py`, BP+DR primary). Between-block mean |construct r|:
...
- **Study C — longitudinal coherence (V0→V1→V2)** (`scripts/11_longitudinal_coherence.py`).
...
- **Study D — predictive validity vs DSM (the make-or-break): MODEST, functioning-specific**
  (`scripts/12_predictive_validity.py`; out-of-sample CV; M0=age+sex+V0-baseline). Attrition check
...
- **Study D refined — relapse done right** (`scripts/13_predictive_survival.py`): the change-based
