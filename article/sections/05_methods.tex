...
covariates $c_i$ that adjust an indicator's \emph{mean} but are never factor indicators. The
reported (primary) fit z-scores each indicator and omits the covariate term; covariate adjustment
(age, sex, education, site; the $\beta_j^{\top}c_i$ / $Bc_i$ term, implemented by residualizing each
indicator on those covariates before the factor model, Frisch--Waugh--Lovell-equivalent for Gaussian
items) is a \emph{sensitivity} arm, and the biology--$G$ result is reported under both --- the
adjustment \emph{strengthens} it (metabolic--$G$ $0.12\!\to\!0.06$; SI). Stacking one patient's items,
...
\emph{ESEM} because an indicator is additionally \emph{allowed} small cross-loadings on its
plausibly-related factors, shrunk toward zero by the soft priors \eqref{eq:softprior}---unlike
confirmatory factor analysis, which forbids cross-loadings outright. The reported fit is a
\emph{Bayesian sparse bifactor with selected ESEM windows} rather than a fully free rotation: cells
marked \textsc{unlikely} are fixed at (equivalently, shrunk to) zero and specific$\leftrightarrow$specific
cross-loadings are not freed (they are rotationally aliased with $\Phi$); a soft-zero sensitivity that
instantiates the $\mathcal{N}(0,0.05^2)$ prior on the unlikely cells reproduces the map to loading
congruence $0.97$ (SI). Two identifications of the factor
...
ordered-normal cutpoints $\tau_{j\cdot}$; the negative-binomial concentration $\alpha_j\sim\mathrm{HalfNormal}(2)$; and
...
($0.12$ and $0.07$) while cognition and sleep are far larger ($0.39$ and $0.42$). The
...
full-$N$ continuous backbone met this strict gate; the joint nine-dimension (mixed)
map did not certify at full $N$ and is reported at the largest sample that mixes
($\hat{R}\le1.04$, effective sample size $\ge112$), with a cross-seed stability guard
(Tucker congruence $\varphi=0.993$).
