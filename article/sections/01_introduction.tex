...
worst-prognosis pole for functioning.

Methodologically, FACE-ATLAS is a latent-variable dimensional measurement model that combines mixed and Gaussian-copula observation layers with an observed-cell likelihood for structured missingness~\citep{reise2012bifactor,asparouhov2009esem,muthen2012bayesian}: routine biological load is placed inside a transdiagnostic psychiatric factor space, its separation from functional burden is measured rather than assumed, and its temporal and prognostic implications are tested without converting a continuum into biotypes.
