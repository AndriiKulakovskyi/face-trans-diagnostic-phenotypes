...
selected $A=5$. The five corners are: $A_0$ \emph{activation/sleep}
($\uparrow$sleep $+2.8$, $\uparrow$mania $+1.7$; BP-enriched, $N=1{,}448$); $A_1$
\emph{severe, clean-biology} ($\uparrow$severity $+1.9$, $\downarrow$immunometabolic
$-1.9$, $\downarrow$developmental; $N=1{,}584$); $A_2$ \emph{immunometabolic burden}
($\uparrow$immunometabolic $+3.5$, $\uparrow$severity $+2.4$, $\uparrow$suicidality;
$N=1{,}426$)---the biology corner; $A_3$ \emph{trauma/suicidality} ($\uparrow$developmental
$+2.8$, $\uparrow$suicidality $+1.4$; $N=2{,}004$); and $A_4$ \emph{low-burden/well}
($\downarrow$sleep $-2.7$, $\downarrow$severity $-2.0$, $\downarrow$mania; $N=2{,}551$).
The symptom space resolves into two distinct corners---activation/sleep ($A_0$) and
trauma/suicidality ($A_3$)---and the biology corner ($A_2$) is the merged immunometabolic
axis. Because immunometabolic load varies continuously without a density gap, it is a
\emph{direction of maximal phenotype}---an archetypal corner---rather than a partition
boundary, which is why the archetypes (and the continuous coordinates), not any
tessellation, are the load-bearing object for biology.
