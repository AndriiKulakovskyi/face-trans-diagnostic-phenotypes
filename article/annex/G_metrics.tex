...
\addlinespace
Factor loading ($\lambda$) & Strength with which one indicator reflects a latent axis (a standardized weight) & $|\lambda|{>}0.3$ substantial; sign gives direction & e.g.\ BMI$\to$immunometabolic $0.95$, CRP $0.37$, FAST$\to$\Gfac{} $0.91$ (all $37$ immunometabolic loadings in Table~\ref{tab:immet_loadings}) \\
\addlinespace
...
\addlinespace
Hopkins statistic & Cluster \emph{tendency} of the cloud & $0.5$ = uniformly random; $\to1$ = clustered & $\to0.5$ (no tendency) \\
\addlinespace
...
\addlinespace
Adjusted Rand Index (ARI) & Agreement between the map's partition and DSM-5 diagnosis & $0$ = chance; $1$ = identical & $0.006$ vs DSM-5 (transdiagnostic) \\
\addlinespace
...
\addlinespace
Functional-remission rate & Share reaching $2$-year functional remission, by archetype corner & The clinical readout of the gradient & $17\%$ (immunometabolic corner) to $52\%$ (low-burden pole) \\
\addlinespace
...
\addlinespace
\multicolumn{4}{@{}l}{\textbf{\textit{F. What can be said about treatment?} (causal bounds, M5 --- exploratory)}} \\
\addlinespace
E-value & How strong an unmeasured confounder would have to be to explain away an observed effect & Higher = more robust; ${\sim}1.2$ = fragile & Lithium-BP $1.20$--$1.28$; antipsychotic-BP $1.80$ \\
\addlinespace
Minimum detectable effect (MDE) & Smallest treatment-moderation the design could have detected & Small = a null is \emph{informative}, not just underpowered & ${\approx}0.20$ SD (lithium); makes the lithium null a \emph{bounded} one \\
\addlinespace
Propensity-score overlap & Whether treated and untreated patients are comparable enough to contrast & High overlap = estimable; low = channelled & Near-complete (lithium); channelled, non-estimable (clozapine) \\
\addlinespace
Attenuation under treatment adjustment & How much the prognostic signal shrinks once the drugs patients received are controlled for & Small = not a treatment artefact & $4$--$8\%$ for the immunometabolic carrier \\
\addlinespace
Treatment-course $\Delta$AUC & Discrimination of $2$-year treatment course by archetype & Small here & Up to ${\approx}+0.04$ (response, side-effects); treatment-resistance marginal \\
\end{longtable}
}
