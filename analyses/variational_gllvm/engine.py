...
stochastic variational inference (PyTorch, :mod:`analyses.variational_gllvm.gllvm`).
...
``immunometabolic`` factor (prior matrix ``loading_matrix.immunometabolic_crossload.csv``),
...
from analyses.variational_gllvm.gllvm import (
    GLLVMTrainer,
    LoadingOntology,
    TrainingConfig,
    VariationalGLLVM,
...
BIOMERGE_XC = REPO / "configs" / "loading_matrix.immunometabolic_crossload.csv"
RESULTS = REPO / "results" / "analyses" / "variational_gllvm"
FIGURES = REPO / "docs" / "figures" / "sensitivity" / "variational_gllvm"
MODEL_VERSION = "variational_gllvm"
    ...
    def build(
        ...
        *,
        ...
        if n_subsample and n_subsample < len(baseline):
            ...
            if balanced:
                from face.measurement.engine import _balanced_idx
...
class GLLVMRunner:
    ...
    def consolidate(self, fit: dict | None = None) -> Path:
        ...
        try:
            from analyses.variational_gllvm import validate as _V
