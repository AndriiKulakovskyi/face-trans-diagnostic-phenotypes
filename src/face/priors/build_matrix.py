...
Outputs (configs/):
  * loading_matrix.csv   — one row per (item, factor) cell
...
def load_configs(configs_dir: Path | None = None) -> tuple[dict, dict, dict]:
    ...
    # likelihood_map.yaml is the hand-maintained SOURCE; likelihoods.yaml is a derived
    # artifact this builder EMITS (never reads back — that would let it go stale).
    liks = _load("likelihood_map.yaml")["likelihoods"]
...
def build_prior_matrix(configs_dir: Path | None = None,
    ...
    if out_csv is None:
        out_csv = CONFIGS / "loading_matrix.csv"
...
def _write_likelihoods_yaml(lik_view: dict[str, str]) -> None:
    ...
    header = ("# configs/likelihoods.yaml — canonical per-item likelihood family.\n"
              "# Generated by src/face/priors/build_matrix.py from likelihood_map.yaml +\n"
